                ...
                else if (actionType.Equals("createFolder"))
                {
                    using (ClientContext clientContext = getClientContext(removeHttpsFromUrl((string)action["ParentSiteUrl"])))
                    {
                        ...
                        if (action.ContainsKey("ParentFolder"))
                        {
                            ...
                            newFolder = parent.Folders.AddWithOverwrite((string)action["FolderName"], true);
                        }
                        else
                        {
                            ...
                            newListItem.Update();
                            //Folder NewF=list.RootFolder.Folders.Add((string)action["FolderName"]);
                            //newFolder = list.RootFolder.Folders.AddWithOverwrite((string)action["FolderName"], true);
                        }
                        //newFolder.Update();
                        //clientContext.Load(newListItem.FieldValues);
                        //clientContext.Load(newFolder, nf => nf.ListItemAllFields);
                        //Console.WriteLine("Folder created with id={0}", newFolder.ListItemAllFields.Id);
